...
class BoostTemperature:
    ...
    @classmethod
    def boot_temperature_converter(cls, input_value: Union[bool, float]) -> 'BoostTemperature':
...
class RoomSetTemperatures:
    ...
    def __getitem__(self, key: TemperatureMode) -> Decimal:
        if key == TemperatureMode.BOOST:
            raise ValueError("Boost temperature can't be accessed through the dictionary method")
...
class IstabaiClient:
    ...
    def __init__(
            self, base_url: str = "https://api.istabai.com"
    ) -> None:
    ...
    async def login(self,
        ...
        try:
            if api_key is not None and (username is not None or password is not None):
                raise IstabaiInvalidApiUsage("Either send api_key or credentials not both")
            if api_key is None and username is None and password is None:
                raise IstabaiInvalidApiUsage("You must send either the credentials or the api key")
            ...
            if api_key is not None:
                ...
                raise Exception("Unexpected situation, should have been handled by validation")

            url = f"{self._base_url}/2/login.json"
            response = await self.session.get(url, params=params)
            login_response = self._check_for_errors_and_deserialize(response, data_class=LoginResponse)
    ...
    async def get_homes(self) -> HomesResponse:
        ...
        return self._check_for_errors_and_deserialize(response, data_class=HomesResponse)
    ...
    async def get_rooms(self, home_id: int) -> RoomsResponse:
        ...
        response = await self.session.get(url, params=params)
        return self._check_for_errors_and_deserialize(response, data_class=RoomsResponse)
    ...
    async def set_boost_temperature(self, room_id: int, temperature: Decimal, duration: int) -> SetTemperatureResponse:
        return await self._set_temperature(room_id, TemperatureMode.BOOST, temperature, duration)

    async def clear_boost(self, room_id: int) -> SetTemperatureResponse:
        return await self._set_temperature(room_id, TemperatureMode.BOOST, Decimal(1), 0)

    async def use_schedules(self, room_id: int, use_schedules: bool) -> UseSchedulesResponse:
        params = {
            "api_key": self._api_key,
            "room_id": room_id,
        ...
        response = await self.session.get(url, params=params)
        return self._check_for_errors_and_deserialize(response, data_class=UseSchedulesResponse)

    async def _set_temperature(self, room_id: int, mode: TemperatureMode, temperature: Decimal,
                               duration: Optional[int] = None) -> SetTemperatureResponse:
        ...
        if mode != TemperatureMode.BOOST and duration is not None:
            raise IstabaiInvalidApiUsage("Duration not allowed in any other mode than boost")
        ...
        response = await self.session.get(url, params=params)
        return self._check_for_errors_and_deserialize(response, data_class=SetTemperatureResponse)
