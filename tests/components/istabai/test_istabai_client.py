...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("When sending the credentials you must provide both the username and password", None, "password", None),
        ("When sending the credentials you must provide both the username and password", "user", None, None),
    ],
)
...
async def test_login_other_response_non_json() -> None:
    ...
    with patch("httpx.AsyncClient.get") as get_mock:
        def get_side_effect(url, **kwargs):
            ...
            return httpx.Response(200, request=request, content=content, headers=headers)
...
async def test_set_temperature_returns() -> None:
    ...
    with patch("httpx.AsyncClient.get") as get_mock:
        ...
        client = IstabaiClient()
        set_temperature_response = await client.set_temperature(204, TemperatureMode.ECO, Decimal('17.2'))
